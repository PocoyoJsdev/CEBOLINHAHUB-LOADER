...
-- _G.__userKey = nil
-- ╔══════════════════════════════════════════╗
-- ║        CEBOLINHA HUB - Anti-Copy         ║
-- ║         Desenvolvido por POCOYO.JS       ║
-- ╚══════════════════════════════════════════╝
local _cebKey = "4a97ed579acec503682c7e74e4c9365e";
local function _cebVerify()
	if not _G.__cebToken then
		warn("❌ CEBOLINHA HUB: Execute apenas pelo loader oficial!");
		game:GetService("Players").LocalPlayer:Kick("⚠️ Use o loader oficial do CEBOLINHA HUB.");
		return false;
	end;
	local HttpService = game:GetService("HttpService");
	local expected = HttpService:UrlEncode(_G.__cebToken .. _cebKey);
	if _G.__cebSign ~= expected then
		warn("❌ CEBOLINHA HUB: Token inválido!");
		game:GetService("Players").LocalPlayer:Kick("⚠️ Loader não autorizado.");
		return false;
	end;
	local now = os.time();
	if math.abs(now - tonumber(_G.__cebTime or 0)) > 300 then
		warn("❌ CEBOLINHA HUB: Token expirado!");
		game:GetService("Players").LocalPlayer:Kick("⚠️ Recarregue o loader.");
		return false;
	end;
	_G.__cebToken = nil;
	_G.__cebSign = nil;
...
if not _cebVerify() then return end;
print("✅ CEBOLINHA HUB carregado com sucesso!");
...
-- print("--------------------------------------------------------------------------");
local Update = (loadstring(game:HttpGet("https://cebolinha-hub-blox.vercel.app/fruitsource.lua")))();
...
local Library = Update:Window({
	SubTitle = "DEV: POCOYO.JS",
	Size = UDim2.new(0, 450, 0, 300),
	TabWidth = 140
});
-- PATCH: Substituir nome NoxHub e logo pela identidade CEBOLINHA HUB
spawn(function()
	task.wait(0.5);
	pcall(function()
		for _, gui in pairs(game:GetService("CoreGui"):GetDescendants()) do
			if gui:IsA("TextLabel") and gui.Text == "NoxHub" then
				gui.Text = "CEBOLINHA HUB";
			end;
			if gui:IsA("ImageLabel") and (string.find(tostring(gui.Image), "nox") or string.find(tostring(gui.Image), "Nox")) then
				gui.Image = "rbxassetid://130502669139756";
			end;
		end;
		...
		-- Loop contínuo para capturar mudanças dinâmicas
...
(getgenv()).Load = function()
	if readfile and writefile and isfile and isfolder then
		if not isfolder("CebolinhaHub") then
			makefolder("CebolinhaHub");
		end;
		if not isfolder("CebolinhaHub/Blox Fruits/") then
			makefolder("CebolinhaHub/Blox Fruits/");
		end;
		if not isfile(("CebolinhaHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
			writefile("CebolinhaHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(_G.Settings));
		else
			local Decode = (game:GetService("HttpService")):JSONDecode(readfile("CebolinhaHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json"));
	...
	if Update:SaveSettings() then
		if readfile and writefile and isfile and isfolder then
			if not isfile(("CebolinhaHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json")) then
				...
				writefile("CebolinhaHub/Blox Fruits/" .. game.Players.LocalPlayer.Name .. ".json", (game:GetService("HttpService")):JSONEncode(Array));
			end;
		else
			return warn("Status : Undetected Executor");
		end;
...
spawn(function()
	...
	wait(1);
	((require(game.ReplicatedStorage.Notification)).new("<Color=Purple>[ Bem vindo ao CEBOLINHA HUB, " .. game.Players.LocalPlayer.DisplayName .. "! ]<Color=/>")):Display();
	wait(1);
	((require(game.ReplicatedStorage.Notification)).new("<Color=Yellow>[ Divirta-se com o CEBOLINHA HUB! ]<Color=/>")):Display();
end);
function CheckQuest()
...
MainTab:Button("Copy Discord CEBOLINHA HUB", function()
	setclipboard("https://discord.gg/z5DhUvEx6Z");
